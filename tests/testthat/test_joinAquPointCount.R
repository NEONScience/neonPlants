...
### Read in test data ####
testList <- readRDS(testthat::test_path("testdata", "apc_testDat_pointCount.rds"))
...
pointCountOutputs <- neonPlants::joinAquPointCount(inputDataList = testList)
...
testthat::test_that(desc = "Output class list input", {
  ...
  testthat::expect_s3_class(object = pointCountOutputs$apc_pointCount,
                            class = "data.frame")
})
...
testthat::test_that(desc = "Output class table input", {
  ...
  temp <- joinAquPointCount(inputPoint = testPoint,
                            inputPerTax = testPerTax,
                            inputTaxonomy = testTaxProc,
                            inputMorph = testMorph)

  testthat::expect_s3_class(object = temp$apc_pointCount,
                            class = "data.frame")
})
...
testthat::test_that(desc = "Output data frame row number list input", {
  ...
  testthat::expect_identical(object = nrow(pointCountOutputs$apc_pointCount),
                             expected = as.integer(145))

  testthat::expect_identical(object = ncol(pointCountOutputs$apc_pointCount),
...
testthat::test_that(desc = "Output data frame row number table input", {
  ...
  testthat::expect_identical(object = nrow(temp$apc_pointCount),
                             expected = as.integer(145))

  testthat::expect_identical(object = ncol(temp$apc_pointCount),
                             expected = as.integer(74))
})
...
testthat::test_that(desc = "Output data frame source: taxonomyProcessed", {
  ...
  outDF <- pointCountOutputs$apc_pointCount
...
testthat::test_that(desc = "Output data frame source: taxonomyRaw", {
  ...
  temp <- joinAquPointCount(inputDataList = tempTestList)

  outDF <- temp$apc_pointCount
