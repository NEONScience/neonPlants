...
herbMassOutputs <- neonPlants::scaleHerbMass(inputDataList = hbpTestDF)
...
testthat::test_that(desc = "Output type", {
  testthat::expect_type(object = herbMassOutputs,
                        type = "list")
})
...
testthat::test_that(desc = "Argument 'inputDataList' is list object", {
  testthat::expect_error(object = neonPlants::scaleHerbMass(inputDataList = hbpTestDF$hbp_perbout),
...
testthat::test_that(desc = "Required tables present in 'inputDataList' input", {
  testthat::expect_error(object = neonPlants::scaleHerbMass(inputDataList = hbpTestDF[1:1]),
...
testthat::test_that(desc = "Table 'inputBout' missing column", {
  ...
  temp <- hbpTestDF
  temp$hbp_perbout <- temp$hbp_perbout %>%
    dplyr::select(-"clipArea")

  testthat::expect_error(object = neonPlants::scaleHerbMass(inputDataList = temp),
...
testthat::test_that(desc = "Table 'inputMass' missing column", {
  ...
  temp <- hbpTestDF
  temp$hbp_massdata <- temp$hbp_massdata %>%
