...
estimateWoodMass = function(inputDataList, 
  ...
  vst_agb$plantStatus2 <- ifelse(vst_agb$plantStatus %in% c("Live", "Live, disease damaged", "Live, insect damaged", "Live,  other damage", "Live, physically damaged","Live, broken bole"), 
    ...
    vst_agb$ninetyCrownDiameter <- dplyr::if_else(vst_agb$ninetyCrownDiameter < 0 , NA, vst_agb$ninetyCrownDiameter)
...
# combine basal area of shrubs belonging to same individualID and calculate round diameter equivalant from that
  ...
  shrub <- shrub %>% 
    ...
    dplyr::summarise(basalArea = sum(.data$basalArea, na.rm = TRUE),
                    ...
                    .groups = "drop")
  # group multiple stems belonging to same individualID, sum basal area, and take the max of the other quantitative values
  
    shrub$basalStemDiameter <- 2*(sqrt(shrub$basalArea/pi)) # round diameter equivalent of the the combined stems belonging to the same individualID
    shrub$stemDiameter <- 2*(sqrt(shrub$basalAreaDBH/pi)) # round DBHdiameter equivalent of the the combined stems belonging to the same individualID
