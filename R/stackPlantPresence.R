...
#'   * variables - Units and definitions of novel variables created by the function that are not already defined in the Plant Presence and Percent Cover data product.
#'
#' @references
...
#' \dontrun{
#'
#' #   Obtain NEON Plant Presence and Percent Cover data; note that a token is required and may be obtained after creating a NEON user accounts get data
#' div <- neonUtilities::loadByProduct(
#'   dpID = "DP1.10058.001",
#'   site = "SRER",
#'   startdate = "2010-01",
#'   package = "basic",
...
#'   )
#'
#' #   Stack the data by using the list returned by neonUtilities::loadByProduct as an input
#' data_stacked <- neonPlants::stackPlantPresence(
#'   inputDataList = div)
#'
...
#'   dplyr::filter(namedLocation == "SRER_043.basePlot.div")
#'
#' my_10_100m_data_SRER_43 <- my_10_100m_data %>%
#'   dplyr::filter(namedLocation == "SRER_043.basePlot.div")
#'
#' data_stacked_SRER_43 <- neonPlants::stackPlantPresence(
#'   inputDataList = list(
...
stackPlantPresence <- function(inputDataList,
  ...
  variables <- variables %>%
    dplyr::filter(.data$functionName == "stackPlantPresence")



  ### Return output ####
  outputs <- list(div_stackedPresence = divPlantPresenceData,
                  variables = variables)
