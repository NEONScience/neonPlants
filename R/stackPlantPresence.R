...
#' 
#' @description Data inputs are NEON Plant Presence and Percent Cover data (DP1.10058.001) retrieved with neonUtilities::loadByProduct() function (preferred), data downloaded from the NEON Data Portal, or input data tables with an equivalent structure and representing the same site x month combinations. The stackPlantPresence() function aggregates the occurrence data from the Plant Presence and Percent Cover data product data product to return the plant species present at each subplot (1m2, 10m2, and 100m2) and the plot scale (400m2). As downloaded from the NEON data portal, data at 10m2 and 100m2 typically include only species not encountered within nested, finer grained subplots, and a list of present species for the entire 400m2 scale plot is not provided.
#' 
