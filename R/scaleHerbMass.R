...
scaleHerbMass = function(inputDataList,
  ...
  ##  Remove samplingImpractical records, and remove exclosure == "Y" records at sites where exclosures were trialed but not used (SRER) and where exclosure == "Y" was mistakenly selected
  ...
  ##  Apply NA when all data are missing, define zeroes when data exist in at least one herbGroup column
  #   Define columns to assess whether all are NAs
  ...
  ### Separate records into grazed, cropped, and standard 'siteID x year' data frames; each requires custom logic to calculate peak standing biomass at the plot level (next section).
  ...
  siteDF <- plotDF %>%
    ...
    dplyr::summarise(herbEventID = paste(unique(.data$eventID), collapse = ", "),
                     herbPlotCount = length(stats::na.omit(.data$herbTotalMass_gm2)),
                     ...
                     herbTotalSD_Mgha = round(stats::sd(.data$herbTotalMass_Mgha, na.rm = TRUE),
                                              digits = 2),
                     herbTotalSEM_Mgha = round(stats::sd(.data$herbTotalMass_Mgha, na.rm = TRUE) / sqrt(.data$herbPlotCount),
                                               digits = 2),
                     ...
                     herbTotalSD_gm2 = round(stats::sd(.data$herbTotalMass_gm2, na.rm = TRUE),
                                             digits = 2),
                     herbTotalSEM_gm2 = round(stats::sd(.data$herbTotalMass_gm2, na.rm = TRUE) / sqrt(.data$herbPlotCount),
                                              digits = 2),
                     .groups = "drop") %>%
    dplyr::arrange(.data$domainID,
                   .data$siteID,
