  ...
  siteMass <- plotMass %>%
    ...
    dplyr::summarise(startDate = min(.data$startDate),
                     ...
                     rootMassMean_gm2 = round(mean(.data$totalMass_gm2, na.rm = TRUE),
                                              digits = 1),
                     rootMassSD_gm2 = round(sd(.data$totalMass_gm2, na.rm = TRUE),
                                            digits = 0),
                     rootMassMean_gm3 = round(mean(.data$totalMass_gm3, na.rm = TRUE),
                                              digits = 1),
                     rootMassSD_gm3 = round(sd(.data$totalMass_gm3, na.rm = TRUE),
                                            digits = 0),
                     rootMassMean_Mgha = round(mean(.data$totalMass_Mgha, na.rm = TRUE),
                                               digits = 4),
                     rootMassSD_Mgha = round(sd(.data$totalMass_Mgha, na.rm = TRUE),
                                             digits = 2),
                     rootPlotNum = dplyr::n(),
                     .groups = "drop")
