...
for(j in 1:length(data_filename_list)){
  ...
  # stack using npe
  d_npe <- npe_longform(j_data_list, scale = "100m", timescale = "subannual")

  # stack using np
  d_np <- neonPlants::stackPlantPresence(j_data_list, totalSampledAreaFilter = 100) %>%
  ...
  for(i in 1:nrow(d_diffs)){
    ...
    # truth
    j_data_list_10_100_recs <- j_data_list$div_10m2Data100m2Data %>%
      filter(
        plotID == my_plotID,
        grepl(
    ...
    j_data_list_1_recs <- j_data_list$div_1m2Data %>%
      filter(
        ...
        grepl(
          gsub("100","",my_subplotID),
          subplotID),        eventID == my_eventID,
        !is.na(taxonID)) %>%
      as.data.frame()

    d_diffs[i,"tax_1m"] <- j_data_list_1_recs$taxonID %>%
      unique() %>% paste(collapse = "|")
    d_diffs[i,"tax_10_100m"] <- j_data_list_10_100_recs$taxonID %>%
      unique() %>% paste(collapse = "|")
    d_diffs[i,"tax_np_10_100m"] <- np_recs$taxonID %>%
      unique() %>% paste(collapse = "|")
    d_diffs[i,"tax_npe_10_100m"] <- npe_recs$taxonID %>%
      unique() %>% paste(collapse = "|")
  ...
  j_out_filename <- paste0(out_dir,"d_diffs_100m_",gsub("\\.RDS","",j_data_filename),".csv")
