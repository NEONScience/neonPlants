...
# stack using npe
d_npe <- npe_longform(D14, scale = "10m", timescale = "subannual")

# stack using np
d_np <- neonPlants::stackPlantPresence(D14, totalSampledAreaFilter = 10) %>%
...
for(i in 1:nrow(d_diffs)){
  ...
  # truth
  d14_10_recs <- D14$div_10m2Data100m2Data %>%
    filter(
      plotID == my_plotID,
      subplotID == my_subplotID,
      eventID == my_eventID) %>%
  ...
  d_diffs[i,"tax_10m"] <- d14_10_recs$taxonID %>%
    unique() %>% paste(collapse = "|")
...
write_csv(d_diffs, file = "d_diffs.csv")
